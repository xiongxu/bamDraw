...
//			fprintf(stdout,"%s: %u\n",(char *)((struct pair *)current->data)->key,chr->gene_count);
			...
			for (j=0;j< chr->gene_count;j++ ) {
				...
				chr_gene_cluster[cluster_index].end=((Gene *)chr_gene_cluster[cluster_index].GeneList->tail->data)->end;
			}
...
//		fprintf(stderr,"$%d\t%d\n",j,(databuf+i)->fp->header->target_len[j]);
	...
	char *current_dir=(char *)malloc(128*sizeof(char));
	memset(current_dir,0,128*sizeof(char));
	getcwd(current_dir, 128*sizeof(char));
	char *work_dir=(char *)malloc(256*sizeof(char));
	memset(work_dir,0,sizeof(256*sizeof(char)));
	
	int color[12],boolen=0;
