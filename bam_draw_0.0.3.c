//gcc -g -O3 -Wall bam_draw.c  -o bam_draw /home/xuxiong/data/bin/samtools-0.1.19/libbam.a  -I/home/xuxiong/data/bin/samtools-0.1.19 -L/home/xuxiong/data/bin/samtools-0.1.19 -lpthread -lgd -lpng -lz -ljpeg -lfreetype -lm
#include <stdio.h>
#include <getopt.h>
#include <math.h>
#include <err.h>
...
static inline void draw_all_regions(struct hash *Clusterd_GTF,DataBuf *databuf,const char *current_dir,char *work_dir);
		...
		while (current) {
			...
			if (current) {
				if (r->n_cigar>1) {
					...
					gdImageSetThickness(im, 4);
					gdImageLine(im, (int)(x+(r->pos-beg+1)*ratio), (int)y, (int)(x+(r->end-beg+1)*ratio),(int)y,(r->strand == '+' ? color[1] : color[10])) ;
				}
...
struct hash *load_gtf(const char *infile){
	...
	if(f == NULL) err(1, "Failed to open file (%s)", infile);
	...
	while (1) {
		...
		gtfline->seqname=(char *)calloc(8,sizeof(char));
		gtfline->feature=(char *)calloc(16,sizeof(char));
		gtfline->attributes=(char *)calloc(128,sizeof(char));
...
void draw_all_regions(struct hash *Clusterd_GTF,DataBuf *databuf,const char *current_dir,char *work_dir){
	unsigned int i,j;
	int color[12];
	for (j=0;j<databuf->fp->header->n_targets ;j++ ) {
		...
		int start=0,end=start+globalArgs.window;
		while (1) {
			int boolen=0;
			fprintf(stderr,"%s\t%d\t%d\n",databuf->fp->header->target_name[j],start,end);
			for (i=0;i<globalArgs.numInfiles ;i++ ) {
				*((databuf+i)->my_list+j)=list_create(Region_delete_int);
				bam_fetch((databuf+i)->fp->x.bam, (databuf+i)->idx,j,start,end,(databuf+i), fetch_func);
				fprintf(stderr,"%s read count in this region: %d\n",*(globalArgs.infiles+i),list_size(*((databuf+i)->my_list+j)));
				if (!list_empty(*((databuf+i)->my_list+j))) boolen=1;
				parse_my_list(databuf+i,j);
			}
			if (boolen) {
				unsigned int max_transcript_num=0,current_top=50;
				int gene_cluster_index=bsearch_draw(chr_culster,start,end,&max_transcript_num);
				gdImagePtr im=creat_png(databuf,globalArgs.numInfiles,color,&max_transcript_num);
				if (gene_cluster_index>0) {
					draw_GeneList(&chr_culster->chr_gene_cluster[gene_cluster_index],max_transcript_num,im,&current_top,start,end,color);
					while (chr_culster->chr_gene_cluster[gene_cluster_index].end <end) {
						draw_GeneList(&(chr_culster->chr_gene_cluster[++gene_cluster_index]),max_transcript_num,im,&current_top,start,end,color);
					}
					draw_GeneList_rectangle(max_transcript_num,im,&current_top,start,end);
				}

				for (i=0;i<globalArgs.numInfiles ;i++ ) {
					if ((databuf+i)->max_depth>0) {
						draw_region((databuf+i)->Stack,im,&current_top,(databuf+i)->max_depth,start,end,color,*(globalArgs.infiles+i));
					}
				}
				write_png(im,(databuf+i-1)->fp->header->target_name[j],start,end,globalArgs.outfile);
			}
			if (end >=(databuf+i-1)->fp->header->target_len[j]) break;
			start+=globalArgs.window - globalArgs.overlap;
			end=start+globalArgs.window;
			end= (end>(databuf+i-1)->fp->header->target_len[j] ? (databuf+i-1)->fp->header->target_len[j] : end);
		}
	}
}
